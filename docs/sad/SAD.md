...
### 3.1 存储介质职责隔离（硬约束，回应 NFR-09）

| 介质 | 唯一职责 | 明确禁止 | 一致性约束 |
|------|---------|---------|-----------|
| MySQL | 业务事实源：账号/组织/RBAC、单元与切片正本、权限规则、FAQ 元数据、访问日志；检索链路仅两处**受控参与**：①关键词召回腿（ngram，LIMIT 20，超时 200ms 熔断）②权限快照未命中时的批量回源 | 不做向量相似检索；不做缓存；不做队列 | 写路径第一顺位；一切派生存储可由它重建 |
| Milvus | 唯一 ANN 相似检索：kb_chunks / faq_vectors；chunk 文本随行返回，检索路径零回表 | 不作为事实源；无跨集合事务 | 派生索引：写路径第二顺位，失败任务可重试；支持一键全量重建 |
| Redis | 易失缓存：权限快照（TTL 300s）、FAQ L1 精确 hash | 不存任何唯一真相——清空只造成性能回退，不丢数据 | 授权表变更经 watermark 自然失效 |
| GPU 三件套 | 无状态推理（chat/embed/rerank） | 不持久化任何业务数据 | 全 env 寻址 + API Key |
| 本地盘 uploads/ | 原始文件暂存 | 解析入库后即非权威数据 | 可随时清理，重建靠 seed 脚本 |

**检索时延预算**（关键路径）：dense ANN ≤10ms ‖ keyword ≤200ms（asyncio 并发，超时弃用并标降级）→ 权限判定 ≤5ms（Redis 快照命中）→ rerank ~30ms → LLM 首 token（百 ms~秒级，主导项）。MySQL 仅出现在被并发掩盖且带熔断的关键词腿与缓存未命中回源，**不在向量检索主路径上，不构成瓶颈**。
